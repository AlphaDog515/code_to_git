 ...
 *  		
		Configuration在读取配置文件的参数时，根据文件名，从类路径按照顺序读取配置文件！
				先读取 xxx-default.xml，再读取xxx-site.xml  //650行
...
public class TestHDFS1 {
	...
	//	fs.mkdirs(new Path("/eclipse2"));
		Path path = new Path ("/demo");
		 ...
		 System.out.println("大小是 : " + fs.getContentSummary(path).getSpaceConsumed());
        // 大小是 : 528
		//  结果是 : 176
          System.out.println("结果是 : " + fs.getContentSummary(path).getLength());
