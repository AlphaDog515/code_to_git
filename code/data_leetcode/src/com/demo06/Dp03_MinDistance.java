...
/*	思路过程可以如下：
	需要求解的是word1编辑到word2所需要的步骤最小值，是不是可以先把word1的一个子串编辑成word2的一个子串？
	然后随着子串的长度逐渐变大，是否可以推导出结果？
	...
	现在来看边界：
	边界是i=0或者j=0；
	如果i=0，表示从horse的一个空子串("")编辑成ros的所有子串(""、"r"、"ro"、"ros")所需要的步数，
	每一个都执行插入就可以了，结果为dp[0][j] = j；
	如果j=0，表示从horse的所有子串(""、"h"、"ho"、"hor"、"hors"、"horse")
	编辑成ros的一个空子串("")所需要的步数，每一步都执行删除就可以了，结果为dp[i][0]=i；
*/
class Solution_3_1 {
