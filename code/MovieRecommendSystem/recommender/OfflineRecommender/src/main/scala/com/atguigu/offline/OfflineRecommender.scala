...
// 基于评分数据的LFM，只需要rating数据  mlib中有一个类是Rating
...
// 定义基于LFM电影特征向量的电影相似度列表  一个电影与其相似的一组电影
...
object OfflineRecommender {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val (rank, iterations, lambda) = (200, 5, 0.1)  // rank 隐特征维度k iterations 迭代次数 lambda正则化系数
    ...
    val movieRecs = movieFeatures.cartesian(movieFeatures)
      ...
      .map {
        case (mid, items) =>
          MovieRecs(mid, items.toList.sortWith(_._2 > _._2).map(x => Recommendation(x._1, x._2)))
      }
      .toDF()
  ...
  def consinSim(movie1: DoubleMatrix, movie2: DoubleMatrix): Double = {
    movie1.dot(movie2) / (movie1.norm2() * movie2.norm2())  // norm2 第二范数
  }
