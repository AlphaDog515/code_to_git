...
object StatisticsRecommender {
  ...
  val GENRES_TOP_MOVIES = "GenresTopMovies" // 类别TOP
  ...
  def main(args: Array[String]): Unit = {
    ...
    // 不同的统计推荐结果
    // 1. 历史热门统计，历史评分数据最多，mid，count   // SQL返回的是DF
    ...
    // 注册udf，把时间戳转换成年月格式   数字 注意 默认是秒 转化毫秒 * 1000L 然后再转化为Int
    spark.udf.register("changeDate", (x: Int) => simpleDateFormat.format(new Date(x * 1000L)).toInt)

    // 对原始数据做预处理，去掉uid
    val ratingOfYearMonth =
      spark.sql("select mid, score, changeDate(timestamp) as yearmonth from ratings")
    ratingOfYearMonth.createOrReplaceTempView("ratingOfMonth")

    // 从ratingOfMonth中查找电影在各个月份的评分，mid，count，yearmonth
    val rateMoreRecentlyMoviesDF =
      spark.sql("select mid, count(mid) as count, yearmonth from ratingOfMonth group by yearmonth, mid order by yearmonth desc, count desc")
