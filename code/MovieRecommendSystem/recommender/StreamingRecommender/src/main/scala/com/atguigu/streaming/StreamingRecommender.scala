...
object ConnHelper extends Serializable {
  lazy val jedis = new Jedis("localhost") // 用的时候才会去初始化
...
object StreamingRecommender {
  ...
  val MAX_USER_RATINGS_NUM = 20 //选取多少个评分
  ...
  def main(args: Array[String]): Unit = {
    ...
    val kafkaParam = Map(
      ...
      "auto.offset.reset" -> "latest" // 偏移量的设置
    )


    // 通过kafka创建一个DStream // 创建消费者
